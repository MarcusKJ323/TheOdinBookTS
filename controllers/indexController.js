      ...
      (err, results) => {
        ...
        if (!errors.isEmpty()) {
          ...
          if (req.file) {
            ...
            console.log(results.image.length == 1);
            if (results.image.length == 1) {
              Image.findOneAndDelete({ author: req.user.id }, (err, img) => {
                if (err) throw err;
              });
            }
            const img = new Image();
            img.data.data = imgdata.toString("base64");
            img.data.Contenttype = "image/png";
            img.author = req.user.id;
            img.save((err, img) => {
              if (err) throw err;
            });
          }
